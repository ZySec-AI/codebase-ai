...
const rustStructRe = /^\s*(pub\s+)?struct\s+(\w+)/;
// enum Foo / pub enum Foo
const rustEnumRe = /^\s*(pub\s+)?enum\s+(\w+)/;
// trait Foo / pub trait Foo
const rustTraitRe = /^\s*(pub\s+)?trait\s+(\w+)/;
// impl Foo / impl Trait for Foo
const rustImplRe = /^\s*impl(?:<[^>]*>)?\s+(?:(\w+)\s+for\s+)?(\w+)/;
// #[test]
const testAttrRe = /^\s*#\[(?:test|cfg\(test\))\]/;
// cfg(test) block
const cfgTestRe = /^\s*#\[cfg\(test\)\]/;
// call sites
const rustCallRe = /\b(\w+)\s*\(/g;
// skip fn declaration lines in call scanning
const rustSkipFnRe = /^\s*(?:pub\s+)?(?:async\s+)?fn\s/;
// skip struct/enum/trait/impl lines in call scanning
const rustSkipDeclRe = /^\s*(?:pub\s+)?(?:struct|enum|trait|impl)\s/;
...
export function parseRust(filePath: string, content: string, root: string): ParseResult {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const fnMatch = rustFnRe.exec(line);
    if (fnMatch) {
    ...
    const structMatch = rustStructRe.exec(line);
    if (structMatch) {
      nodes.push({
    ...
    const enumMatch = rustEnumRe.exec(line);
    if (enumMatch) {
      nodes.push({
    ...
    const traitMatch = rustTraitRe.exec(line);
    if (traitMatch) {
      nodes.push({
    ...
    const implMatch = rustImplRe.exec(line);
    if (implMatch) {
  ...
  for (const line of lines) {
    if (rustSkipFnRe.test(line)) {
      continue;
    }
    if (rustSkipDeclRe.test(line)) {
      continue;
    }
    rustCallRe.lastIndex = 0;
    let cm: RegExpExecArray | null;
    while ((cm = rustCallRe.exec(line)) !== null) {
      const callee = cm[1];
